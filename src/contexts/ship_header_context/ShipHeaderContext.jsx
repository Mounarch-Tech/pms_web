...
export const ShipHeaderProvider = ({ children }) => {
  ...
  const fetchShips = async () => {
    ...
    try {
      ...
      // If office staff → filter allocated ships
      // console.log('user in ship header context : ', user)
      // if (user?.emp_type == 2) {
      //   const officeStaff = officeStaffList.find(
      //     (os) => os.user_id == user.UHA_ID
      //   );

      //   if (officeStaff?.allocated_ships) {
      //     // Keep only ships that are in allocated_ships list
      //     const ships = response.data.filter(s =>
      //       officeStaff.allocated_ships.includes(s.SHA_ID)
      //     );

      //     console.log('Ships in ship header context ::: ', ships)


      //     setShipsList(ships);
      //     return; // Prevents overwriting with full list below
      //   }
      // }
