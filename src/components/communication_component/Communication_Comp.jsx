...
const Communication_Comp = ({ jcd, onClose, refreshJCDPage, refreshTree, refreshPlannedJobs, isOpenByExtention }) => {
    ...
    // To Check LoggedIn Users Permitions to See or send messages
    const [userPermissions, setUserPermissions] = useState({ canView: true, canSend: true });

    // Add this state for grouped messages
    ...
    useEffect(() => {
        ...
        const initCommunication = async () => {
            try {
                ...
                //  Add flag if opened via extension
    ...
    //  Enhanced function to get seen users info
    ...
    //  Enhanced helper function to get user info with designation
    ...
    //  Handle message seen status when sub-tab changes
    useEffect(() => {
    ...
    //  Get seen status with detailed information
    ...
    const loadMessagesForSubTab = async (tab, subTabName, commData = communicationData) => {
        ...
        try {
            ...
            console.log('data of responsed messages ::: ', response.data);

            if (response.data.success) {
                const backendMessages = response.data.messages || [];
                console.log('Backend messages array:', backendMessages);

                // FIX: Simplified message processing
                const allMessages = backendMessages.map(msg => {
                    const isSentByCurrentUser = msg.sender_id === loggedInUserId;

                    // Process attachments - backend already provides proper array
                    const enrichedAttachments = (msg.attachments || []).map(attachmentPath => {
                    ...
                    // Format timestamp - use the timestamp from backend
                    let timestamp;
                    let isoTimestamp = msg.timestamp;

                    if (msg.timestamp) {
                        const messageDate = new Date(msg.timestamp);
                        if (!isNaN(messageDate.getTime())) {
                            timestamp = formatTimestamp(messageDate);
                            isoTimestamp = messageDate.toISOString();
                        }
                    }
                    ...
                    if (!timestamp) {
                        ...
                        timestamp = formatTimestamp(fallbackDate);
                    ...
                    return {
                        ...
                        seenBy: msg.seenBy || [],
                        isSeenByCurrentUser: msg.seenBy?.includes(loggedInUserId) || false,
                ...
                allMessages.sort((a, b) => {
                    const timeA = a.isoTimestamp ? new Date(a.isoTimestamp) : new Date(a.rawTimestamp || a.timestamp);
                    const timeB = b.isoTimestamp ? new Date(b.isoTimestamp) : new Date(b.rawTimestamp || b.timestamp);
                    return timeA - timeB; // Ascending order (oldest first)
                });

                // Group messages by date for better UI organization
                const groupedMessagesData = groupMessagesByDate(allMessages);

                // Update both states
                setMessages(prev => ({
                    ...prev,
                    [sheet.ceh_id]: allMessages
                }));

                setGroupedMessages(prev => ({
                    ...prev,
                    [sheet.ceh_id]: groupedMessagesData
                }));

                console.log(`Loaded ${allMessages.length} messages for sheet ${sheet.ceh_id}`);
            }
        } catch (error) {
    ...
    const groupMessagesByDate = (messages) => {
        ...
        messages.forEach(message => {
            const messageDate = new Date(message.isoTimestamp || message.rawTimestamp || message.timestamp);
            const dateKey = messageDate.toDateString(); // "Mon Nov 11 2024"
    ...
    // Enhanced formatTimestamp function to include date when needed
    ...
    const handleSendMessage = async () => {
        ...
        // FIX: Create proper message structure
        const messageData = {
            text: inputText.trim(),
        ...
        if (attachments.length > 0) {
            ...
            formData.append('message_text', JSON.stringify(messageData)); // Send as JSON string
            formData.append('seenBy', JSON.stringify([]));
            ...
            payload = {
                ...
                message_text: JSON.stringify(messageData), // Send as JSON string
        ...
        const newMessage = {
            ...
            status: "sending",
            seenBy: [], //  Initialize empty seen array
            isSeenByCurrentUser: true //  Current user has seen their own message
        };
    ...
    const renderSubTabContent = () => {
        ...
        switch (activeSubTab.name) {
                ...
                return (
                    <div className="chat-container-whatsapp">
                        ...
                        <div className="chat-messages-scrollable-whatsapp">
                            {/* Use either grouped messages or regular messages */}
                            {groupedMessages[currentSheetId] ?
                                renderMessagesWithDateHeaders(currentSheetId) :
                                ...
                                <div className="chat-input-container-whatsapp">
                                    {(() => {
                                        ...
                                        const isAuthorized =
                                            ...
                                            jcd.activeJobs.secondary_issued_to === user.UHA_ID ||
            ...
            // In your renderSubTabContent function, update the ExtensionA/ExtensionB/ExtensionC case:
            case "ExtensionA":
            case "ExtensionB":
            case "ExtensionC":
                return (
                    <div className="chat-container-whatsapp">
                        ...
                        <div className="chat-sticky-footer-whatsapp">
                            <>
                                {/* ✅ Enhanced Extension Approval/Rejection Logic */}
                                {(() => {
                                    // Find the specific extension for this tab
                                    const currentExtension = extendedJobsList.find(extData =>
                                        extData.jcd_id === jcd.relatedJCD?.JCDSHA_ID &&
                                        extData.JPTA_ID === jcd.activeJobs.JPHA_ID &&
                                        extData.ext_no === activeTab.extNo // Match by extension number
                                    );
                                    ...
                                    const extStatus = currentExtension?.ext_request_status;
                                    const isAuthorizedForDecision = jcd?.activeJobs?.extensions_authority === user.UHA_ID;
                                    const isSheetClosed = currentSheet?.status === 2;
                                    ...
                                    console.log('Extension Debug:', {
                                        ...
                                        isSheetClosed,
                                        jobStatus: jcd.activeJobs.job_status,
                                        extensionsAuthority: jcd.activeJobs.extensions_authority,
                                        currentUserId: user.UHA_ID
                                    });

                                    return (
                                        <>
                                            {/* Show buttons only if:
                                    - User is authorized (extensions_authority)
                                    - Sheet is not closed
                                    - Extension status is pending (1)
                                    - Job status is extension requested (7)
                                */}
                                            {isAuthorizedForDecision &&
                                                !isSheetClosed &&
                                                extStatus === 1 &&
                                                jcd.activeJobs.job_status === 7 && (
                                                    <div className="approval-actions-whatsapp">
                                                        <button
                                                            className="btn-whatsapp approve"
                                            ...
                                            {extStatus === 2 && (
                                                <div className="approval-status-whatsapp approved">
                                                    ✅ Extension Approved
                                                    {currentExtension?.new_execution_deadline && (
                                                        <div className="extension-deadline">
                                                            New Deadline: {new Date(currentExtension.new_execution_deadline).toLocaleDateString()}
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {extStatus === 3 && (
                                                <div className="approval-status-whatsapp rejected">
                                                    ❌ Extension Rejected
                                                    {currentExtension?.denied_on && (
                                                        <div className="extension-denied-date">
                                                            Rejected on: {new Date(currentExtension.denied_on).toLocaleDateString()}
                                                        </div>
                                                    )}
                                                </div>
                                            )}

                                            {/* Show message if user is not authorized but extension is pending */}
                                            {!isAuthorizedForDecision && extStatus === 1 && (
                                                <div className="approval-status-whatsapp info">
                                ...
                                <div className="chat-input-container-whatsapp">
                                    {(() => {
                                        const currentExtension = extendedJobsList.find(extData =>
                                            extData.jcd_id === jcd.relatedJCD?.JCDSHA_ID &&
                                            extData.JPTA_ID === jcd.activeJobs.JPHA_ID &&
                                            extData.ext_no === activeTab.extNo
                                        );
                                        ...
                                        // Allow messaging only if:
                                        // - User is authorized AND
                                        // - Sheet is not closed AND
                                        // - Extension is either pending (1) or approved (2)
                                        const isDisabled = isSheetClosed || !isAuthorized || extStatus === 3;
                                                    ...
                                                    title={
                                                        !isAuthorized
                                                            ...
                                                            : isSheetClosed
                                                                ? "Sheet is closed"
                                                                : extStatus === 3
                                                                    ? "Extension rejected - messaging disabled"
                                                                    : "Attach a file"
                                                    }
                                                    style={{
                                                        cursor: isDisabled ? "not-allowed" : "pointer",
                ...
                return (
                    <div className="report-section-whatsapp">
                        ...
                        <button
                            ...
                            disabled={jcd?.activeJobs?.job_status == 6 || isSheetClosed}
                            style={{
    ...
    const handleDecision = async (key, decision) => {
        console.log('Decision triggered:', { key, decision });

        setApprovals((prev) => ({ ...prev, [key]: decision }));

        try {
            if (key === "ExtensionRequest" && decision === "approved") {
                // Show deadline input modal for extension approval
    ...
    const submitDecision = async (key, decision, deadline = null) => {
        try {
            ...
            // Add deadline if provided (for extension approval)
            if (deadline) {
            ...
            if (response.data.success) {
                ...
                if (typeof refreshExtendedJobsList === "function") await refreshExtendedJobsList();

                // Refresh communication structure to reflect changes
                if (communicationData?.comm_id) {
                    const commResponse = await axios.get(`${API_URL}communication/${communicationData.comm_id}`);
                    if (commResponse.data.success) {
                        setCommunicationData(commResponse.data.commStructure);
                    }
                }
    ...
    const getStatusClass = (status) => {
        if ([1, 2, 3, 4].includes(status)) return "Under-Execution";
        if ([5, 6].includes(status)) return "in-progress";
        if (status === 7) return "extension-requested";
        if (status === 8) return "extension-approved";
        return "unknown";
    };
    ...
    // Enhanced message rendering with date headers
    const renderMessagesWithDateHeaders = (sheetId) => {
        const messages = groupedMessages[sheetId];

        if (!messages || Object.keys(messages).length === 0) {
            return <div className="no-messages">No messages yet</div>;
        }

        return Object.entries(messages).map(([date, dayMessages]) => (
            <div key={date} className="message-date-group">
                <div className="date-header">
                    {getFormattedDateHeader(date)}
                </div>
                {dayMessages.map((msg) => renderMessageBubble(msg, sheetId))}
            </div>
        ));
    };
