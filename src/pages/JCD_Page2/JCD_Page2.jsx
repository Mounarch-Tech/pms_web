...
const JCD_Page2 = () => {
    ...
    // useEffect(() => {
    //     console.log('employeeList in jcd page 2 :: ', employeeList)
    //     console.log('user in jcd page 2 :: ', user)
    // }, [employeeList])
    ...
    //             // UPDATE MODE
    //             filter_jcd_data_linked_on_ships();
    //         } else {
    //             // CREATE MODE
    ...
    const mapBackendToFormData = (data) => {
        ...
        return {
            ...
            jcd_category: data.jcd_category ?? null,
            ...
            job_will_generate_on: data.job_will_generate_on
                ? (Array.isArray(data.job_will_generate_on) ? data.job_will_generate_on : [data.job_will_generate_on])
                : null,
            ...
            secondary_desg: data.secondary_desg ?? null, // frontend-only field
    ...
    const unlinkJcdsFromShip = async (jcdId, ship_id) => {
        // Validate input
        if (!jcdId) {
            toast.warning("No JCD ID provided for unlinking");
            return;
        }
        ...
        const shipName = shipsList.filter(s => s.SHA_ID == ship_id)[0]?.ship_name
        await refreshPlannedJobs()
        await refreshJCDSchedules()

        const jcdshaID = JCD_schedule_List.filter(j => j.jcd_id == jcdId && j.SHA_ID == ship_id)[0]?.JCDSHA_ID
        const activeJob = plannedJobList.filter(pl => pl.jcd_id == jcdshaID)[0]
        // alert(plannedJobList.filter(pl => pl.jcd_id == jcdshaID)[0])
        if (activeJob?.JPHA_ID) {
            toast.error('Active Job is Open Currently, Complete it First then You can able to Unlink it..')
            return
        }
        ...
        try {
            const payload = {
                SHA_ID: ship_id,
                JCD_ID: jcdId  // Send single JCD_ID (string)
            };

            const response = await axios.post(`${apiUrl}unlinkJCDFromShipAndSchedule`, payload);

            if (response.status === 200 || response.status === 201) {
                toast.success(`Successfully unlinked JCD: ${jcdId}`);

                await refreshJCDSchedules()

                // Update assigned JCDs in current ship group
                await RefreshJCD_ship_wise_group_combinations();

                // Update tree because counts are affected
                await refreshTree();

                // Clear from assigned checklist if present
                setAssignedJCDCheckList(prev => prev.filter(id => id !== jcdId));
    ...
    // Upcomming jobs methouds 
    // Add these helper functions before the return statement
    // Calculate next generation function
    ...
    const calculateUpcomingJobs = useMemo(() => {
        if (!selectedNode) return [];

        const upcomingJobs = [];

        // Get JCDs for the selected component
        const componentJCDs = JCD_schedule_List.filter(jcd =>
            jcd.jcd_applied_cat === selectedNode.data.CHA_ID ||
            jcd.jcd_applied_sub_cat === selectedNode.data.SCHA_ID ||
            jcd.jcd_applied_2nd_sub_cat === selectedNode.data.SSCHA_ID ||
            jcd.jcd_applied_3rd_sub_cat === selectedNode.data.TSCHA_ID
        );

        componentJCDs.forEach(jcd => {
            // Find if there's an active job for this JCD
            const activeJob = plannedJobList.find(job =>
                job.jcd_id === jcd.JCDSHA_ID &&
                job.job_status >= 1 &&
                job.job_status <= 5
            );
            ...
            // Get the latest completed job for this JCD
        ...
        return upcomingJobs;
    }, [selectedNode, JCD_schedule_List, plannedJobList]);

    // Filter upcoming jobs based on ship selection and search
    const filteredUpcomingJobs = useMemo(() => {
        let filtered = calculateUpcomingJobs;

        // Filter by ship
        if (selectedShipIdForUpcoming) {
            filtered = filtered.filter(job => job.jcd.SHA_ID == selectedShipIdForUpcoming);
        }

        // Filter by search term
        if (searchTermUpcoming) {
            filtered = filtered.filter(job =>
                job.jcd.jcd_name?.toLowerCase().includes(searchTermUpcoming.toLowerCase()) ||
                job.jcd.jcd_id?.toLowerCase().includes(searchTermUpcoming.toLowerCase())
            );
        }

        return filtered;
    }, [calculateUpcomingJobs, selectedShipIdForUpcoming, searchTermUpcoming]);



    // Render interval cell
        ...
        return filteredUpcomingJobs.map((job, index) => {
            ...
            return (
                <tr key={jcd.jcd_id}>
                    ...
                    <td>{renderIntervalCell(generationInfo?.runningHours, 'running_hours')}</td>
                    <td>
                        {generationInfo?.kmInterval ? (
                            <div style={{ textAlign: 'left' }}>
                                <small style={{ color: '#059669', fontSize: '0.7rem' }}>
                    ...
                    <td>{renderIntervalCell(generationInfo?.periodic, 'periodic')}</td>
                    <td>
                        {generationInfo?.conditional ? (
                            <div style={{ textAlign: 'left' }}>
                                {generationInfo.conditional.conditions.map((condition, idx) => (
    ...
    return (
        <div id="JCD_Page_main_root_container">
                                ...
                                <div id="JCD_configuration_form_container_in_jcd_page">
                                    ...
                                    {profiles?.filter(p => user?.profile_ids?.includes(p.PROFILE_ID))[0]?.form_ids.includes("F_JCD_001") && (
                                        <form onSubmit={handleSubmit} className="jcd-form">
                                            ...
                                            {/* Category */}
                                            <div className="form-group">
                                                <label>Servicable Or Replaceable <span style={{
                                                    color: 'red'
                                                }}>*</span></label>
                                                <select
                                                    name="jcd_category"
                                            ...
                                            {[
                                                ...
                                                <div key={key} className="form-group">
                                                    <label>
                                                        {label} {key != 'secondary_desg' && <span style={{ color: 'red' }}>*</span>}
                                                    </label>
                                                    <select
                                                        name={key}
                                                        ...
                                                        disabled={isReadOnly}
                                                        className="form-control"
                                                        required={key !== 'extension_authority'}
                                                    >
                                                        <option value="">Select</option>
                                        ...
                                        <div id="select-ship-active-jobs-tab">
                                            <select
                                                style={{
                                                    ...
                                                    borderRadius: "10px",  // Fixed typo: was '10x'
                                                    color: "gray",
                                                    width: "fit-content",
                                                    marginTop: "20px"
                                                }}
                                                                        ...
                                                                        {jcd?.activeJobs?.old_issued_to?.length > 0 ? (
                                                                            <button
                                                                                ...
                                                                                onClick={() => {
                                                                                    ...
                                                                                    try {
                                                                                        // Ensure old_issued_to is parsed JSON
                                                                                        temp = jcd.activeJobs?.old_issued_to
                                                                                            ? JSON.parse(jcd.activeJobs.old_issued_to)
                                                                                            : [];
                                                                                    } catch (error) {
                                                                                    ...
                                                                                    setOldDetailsDataOfPeviousEmployees(prev => [
                                                                                        ...prev,
                                                                                        ...temp // 👈 spread so objects are inserted directly
                                                                                    ]);
                    ...
                    {activeTab === "upcoming_jobs" && (
                        <div className="active_tab_main_container">
                            ...
                            <div className="ship-filter-container" style={{ marginBottom: '20px' }}>
                                ...
                                {/* Search Box */}
                                <div className="search-box" style={{ marginLeft: '20px', display: 'inline-block' }}>
                                    <input
                                        type="text"
                        ...
                        <div id='link-JCD-To-Ship-At-Config-Time-content-container'>
                            ...
                            <div className="modal-footer">
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        // Collect applied-from dates along with ship_id
                                        const shipAppliedFromCombination = checkedShipsForNewJcdLinking.map((ship) => {
                                            const appliedFromDate = document.querySelector(
                                                `.applied-from-date[data-ship-id="${ship.SHA_ID}"]`
                                            )?.value || null;
                                        ...
                                        console.log('shipAppliedFromCombination : ', shipAppliedFromCombination)

                                        // Final payload
                                        const payload = {
                                            updatedDataNewJcd: updatedDataForNewJcd, // your raw/master JCD details
                                            ship_appliedFromCombination: shipAppliedFromCombination,
                                        };

                                        // console.log("🚀 Payload to send:", payload);

                                        await axios.post(`${apiUrl}addJcdWithShip_AppliedFrom`, payload)
                                            .then(async (res) => {
                                                if (res.status == 201) {
                                                    ...
                                                    // ✅ 4. Force refresh ALL contexts (critical!)
                                                    await Promise.all([
                                                        refreshJCDSchedules(),
                                                        refreshPlannedJobs(),
                                                        refreshDesignationList(),
                                                        ...
                                                        refreshThirdSubCategoryList(),
                                                        refreshShipsList(),
                                                        refreshTree(), // This ensures tree & selectedNode sync
                                                        refreshJobTypes()
                                                    ]);

                                                    // ✅ 5. Optional: Reset selection
                                                    clearSelection();
                                            ...
                                            .catch((err) => {
                                                ...
                                                toast.error('Failed to create JCD. Server error.');
                                            });
                                    }}
                                >
                                    Confirm & Save
                                    ...
                                    return (
                                        <div>
                                            ...
                                            <div style={{ marginBottom: '24px' }}>
                                                ...
                                                {/* <input
                                                type="date"
                                                id={`applied-from-${checkedShip.SHA_ID}`}
                                                style={{
                                                    width: '100%',
                                                    padding: '10px',
                                                    borderRadius: '6px',
                            ...
                            >
                                ...
                                {currentShipIndex < checkedShipsToAllocatedSelectedJCD.length - 1 ? (
                                    <button
                                        ...
                                        onClick={() => {
                                            const checkedShip = checkedShipsToAllocatedSelectedJCD[currentShipIndex];
                                            const date = document.getElementById(`applied-from-${checkedShip.SHA_ID}`)?.value;
                                    ...
                                    <button
                                        ...
                                        onClick={async () => {
                                            ...
                                            const ship_appliedFromCombination = checkedShipsToAllocatedSelectedJCD.map((ship) => ({
                                                ship_id: ship.SHA_ID,
                                                applied_from: appliedFromDates[ship.SHA_ID] || null, // Use state!
                                            }));

                                            const payload = {
                                                updatedDataNewJcd: selectedJCDForViewShips,
                                            ...
                                            try {
                                                ...
                                                // In the linkExistingJCDWithShips section, update the API call:
                                                const result = await axios.post(`${apiUrl}linkExistingJCDWithShips`, formData, {
                                                    headers: {
                                                        'Content-Type': 'multipart/form-data',
                                                    },
                                                });
                                                console.log('Hii result : ', result)

                                                toast.success('JCD successfully linked to all ships!');
                                                ...
                                                // ✅ 3. Clear ship linking state
                                                setLinkJCDToShipAtConfigTime(false);
                                                setCheckedShipsForNewJcdLinking([]); // if you're using this state

                                                // ✅ 4. Refresh data from backend (via context or API)
                                                refreshJCDSchedules();     // Sync JCD list
                                                refreshPlannedJobs();      // If you have planned jobs
                                                refreshDesignationList();  // If needed
                                                // Add other refresh functions as needed

                                                // ✅ 5. Optional: Reset component selection
                                                // selectNode(null); // Clear selected component in tree
            ...
            {/* PopUp For Old Employees Working On Same Job But Still Not Executed Successfully */}
            {
                isWantToSeeOldDetailsOfPeviousEmployee && (
                    <div id='old-detials-popup-main-container'>
                        <div id='old-detials-popup-content-container'>
                            <table>
                                <thead>
                                    <tr>
                                        <th>SR</th>
                                ...
                                <tbody>
                                    {oldDetailsDataOfPeviousEmployees.length > 0 ? (
                                        oldDetailsDataOfPeviousEmployees.map((oldData, index) => (
                                            <tr key={index}>
                                                <td style={{ textAlign: 'center' }}>{index + 1}</td>
                                                <td style={{ textAlign: 'center' }}>{employeeList.filter(emp => emp.UHA_ID == oldData.emp_id)[0]?.emp_name || 'Emp Name NOT Present In DB'}</td>
                                                <td style={{ textAlign: 'center' }}>{designationList.filter(desg => desg.DSGH_ID == oldData.desg_id)[0]?.desg_name || 'Designation Name Not Present in DB'}</td>
                                                <td style={{ textAlign: 'center' }}>{(oldData.acknoledge_dt)?.split('-').reverse().join('-')}</td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
                                            <td colSpan={4}>No Old Employees</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
