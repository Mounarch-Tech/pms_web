...
const LogInPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // --- 1. Attempt to find the potential user's current ship_id (required by backend route) ---
        // This attempts to provide a ship_id if the user is a known crew member, otherwise sends null.
        const potentialUser = usersList.find(u => u.user_email === email);
        let ship_id_to_send = null;
        if (potentialUser) {
            const shipCrewAssignment = crewData.find(sc => sc.user_id === potentialUser.UHA_ID && sc.crew_status === 1);
            ship_id_to_send = shipCrewAssignment?.ship_id || null;
        }


        try {
            // 2. Call Backend Login API for Authentication and Activity Recording
            const response = await fetch(`${API_BASE}login`, { // Assumed endpoint is /user/login
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    email: email,
                    pin: pinString,
                    ship_id: ship_id_to_send // Send the determined ship_id to be recorded
                }),
            });
            ...
            if (result.success) {
                const user = result.user; // User object from backend (Authenticated User)
                const activity_session_id = result.activity_session_id; // **UDADA_ID from backend for login record**

                // Call backend to auto-activate planned allocations (original logic)
                try {
                    const activationResponse = await fetch(`${API_BASE}activateSuccessor`, {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                    ...
                    console.log("Auto-activation response:", activationResult);
                } catch (activationError) {
                    console.error("Error auto-activating user allocations:", activationError);
                }

                // Find Ship/Office Assignment using the authenticated user's UHA_ID
                const shipCrew = crewData.filter(sc => sc.user_id === user.UHA_ID && sc.crew_status === 1)[0] || null;
                const officeStaff = officeStaffList.filter(os => os.user_id === user.UHA_ID)[0] || null;
                ...
                const employeeData = {
                    ...
                    activity_session_id: activity_session_id // Store UDADA_ID for subsequent logout
                };
                ...
                // Handle error response from backend (Invalid PIN, inactive account, etc.)
                setLoginError(result.message || 'Login failed. Please check your details.');
            }
        } catch (error) {
