...
const DepartmentDesignation = () => {
  ...
  async function officeStaffListRefresh() {
    if (officeStaffList) {
      ...
      <div className="slate-dd-control-panel">
        ...
        <div className="slate-dd-select-card">
          ...
          >
            ...
            {officeStaffList.filter(staff => staff.user_id === user.UHA_ID).flatMap(staff => shipsList.filter(ship => staff.allocated_ships.includes(ship.SHA_ID))).map(ship => (<option key={ship.SHA_ID} value={ship.SHA_ID}> {ship.ship_name} </option>))}
          </select>
        </div>
      </div>
